...
template <class Type,class Counter = int> class Vector {
	...
	public:
		...
		//! constructor initializing vector size items of t item
		...
		//! set vector to given vector (resize, copy data)
		...
		// allocate size items, set length to size
		void resize(int size) {
			allocate(size);
			length = size;
		}
		
		// if size > capacity assign new memory(size) and copy data
		...
		// if size > capacity assign new memory(2*size) and copy data
		...
		//! set length and capacity to 0, free memory
		void destroy() {
			int dynamic = (capacity & DYNAMIC_FLAG);
			length = 0;
			capacity = 0;
		...
		//! append item at end of vector without checking capacity
		INLINE void appendFast(const Type &t) {
			data[(int)(length++)] = t;
		}

		//! append item at given position
		...
		//! remove item at end (decrement length)
		INLINE void remove() {
			assert(length > 0 && "Vector::remove(): bad length");
			length--;
		}

		//! remove size items at given position
		...
		//! copy end item to item at position and decrement length
